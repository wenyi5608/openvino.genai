...
if __name__ == '__main__':
    ...
    if model_reduce_logits:
        if not LLM_MODEL_OV_INT4_REDUCE_LOGITS.exists() and LLM_MODEL_OV.exists():
            print("###### Compress FP16 reduce logits model to INT4 model")
            ...
            ov_compressed_model = nncf.compress_weights(ov_model, **compression_configuration, backup_mode=BackupMode.NONE)
            ov.save_model(ov_compressed_model, LLM_MODEL_OV_INT4_REDUCE_LOGITS)
    else:
        if not LLM_MODEL_OV_INT4.exists() and LLM_MODEL_OV.exists():
            print("###### Compress FP16 model to INT4 model")
            compression_configuration = {
                "mode": nncf.CompressWeightsMode.INT4_SYM,
                "group_size": 32
                }
